...
public class SettingsFrame extends JFrame implements ActionListener
{
	...
	public SettingsFrame(CustomerFrame cframe)
	{
		super("User Settings");
		...
		uidTF = new JTextField(customer.getUserId());
		...
		nidTF = new JTextField(customer.getNid());
		...
		int birthIndex[] = Common.getBirthIndex(customer.getBirthdate());
		...
		if(customer.getGender().equals("Female")) genderCB.setSelectedIndex(1);
		...
		phoneTF = new JTextField(customer.getPhone());
		...
		if(ae.getSource() == changeButton)
		{
			...
			if(password != null && password.equals(customer.getPassword()))
			{
				...
				if(newPasswordPF.getText().length()>0)
				{
					...
					customer.setPassword(newPasswordPF.getText());
				}
				...
				customer.setGender(Common.genders[genderCB.getSelectedIndex()]);
				customer.setEmail(emailTF.getText());
				customer.setPhone(phoneTF.getText());

				UserRepository userRepository = new UserRepository();
				String successReport = userRepository.updateUser(customer.getUser());
